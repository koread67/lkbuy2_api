...
EXHAUST_SELL_BONUS = 15      # 상승 피로 구간 매도 가점
...
def generate_signal(indicators: dict, decision: str) -> dict:
    ...
    low_vix_risk = bool(pd.notna(vix) and pd.notna(vix_5ma) and _clean_float(vix) < _clean_float(vix_5ma) * 0.92)
    ...
    exhaustion_risk = bool(
        high20_dist_f >= SELL_HIGH_NEAR
        and ((rsi_f >= 65 and rsi_delta_f <= 0) or (cci_f >= 100 and cci_delta_f < 0) or ma5_delta3_f <= 0)
    )
